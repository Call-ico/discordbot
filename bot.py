...
from pparser import fetch_iccup_stats
...
# Команда /stats объединяющая всё вместе
@tree.command(name="stats", description="Показать статистику игрока ICCup")
...
async def stats(interaction: discord.Interaction, nickname: str):
    ...
    try:
        ...
        draw_online_status_icon(
            background,
            online_status,
            position=(178, 446),  # координаты, куда вставить иконку
            icon_size=(69, 25),
            icon_online_path="online.png",  # путь к иконке Online
            icon_offline_path="offline.png",  # путь к иконке Offline
            icon_unknown_path="unknown.jpg"  # путь к иконке Unknown (если статус не определён)
        )
        draw_k_value_on_image(background, k_value,
                              position=(42, 167),  # новые координаты
                              font_path="arial.ttf",  # другой шрифт
                              font_size=25,  # больший размер
                              fill=(255, 235, 130),
                              stroke_width = 3,
                              stroke_fill = (0, 0, 0)
        )
        ...
        # Создание прогресс-бара винрейта
        win_lose_leave = data.get("Победы / Поражения / Ливы", "—")
        winrate_percent_str = data.get("Победы %", "—")

        progress_percent = 0

        # Используем готовый процент винрейта
        if winrate_percent_str != "—":
            try:
                progress_percent = float(winrate_percent_str.replace('%', '').strip())
                progress_percent = min(100, max(0, progress_percent))
            except (ValueError, AttributeError):
                progress_percent = 0
        # Если готового процента нет, рассчитываем сами
        elif win_lose_leave != "—":
            try:
                parts = [part.strip() for part in win_lose_leave.split('/')]
                if len(parts) >= 2:
                    wins = int(parts[0])
                    losses = int(parts[1])
                    ...
                    if total_games > 0:
                        progress_percent = (wins / total_games) * 100
                        progress_percent = min(100, max(0, progress_percent))
            except (ValueError, IndexError):
                progress_percent = 0

        # Создаем и вставляем прогресс-бар
